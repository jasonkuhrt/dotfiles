...
type DeepCheck<$S> = $S extends `${infer __c__}${infer __rest__}`
  ? string extends __rest__
    ? number
    : __rest__ extends ""
      ? 1
      : __rest__ extends `${infer __c2__}${infer __r2__}`
        ? string extends __r2__
          ? number
          : __r2__ extends ""
            ? 2
            : never
...
type Bad<$T> = $T extends string
  ? "string" // ← Alignment lost
  : $T extends number
    ? "number" // ← Alignment lost
    : "unknown";

// ❌ BAD - Using nested when flat would work
type Bad2<$T> = $T extends string
  ? "string"
  : $T extends number
    ? "number" // ← Unnecessary nesting
    : $T extends boolean
      ? "boolean"
      : "unknown";
```
