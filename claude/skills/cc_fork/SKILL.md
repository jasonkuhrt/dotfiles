---
name: cc:fork
description: Use when delegating tasks to parallel Claude agents, spawning independent sessions, or forking work to run concurrently. Triggers on "spawn", "fork", "delegate to another claude", "run in separate terminal", "launch parallel agent".
---
...
1. Run the fork script:
   ...
   ```bash
   ~/.claude/skills/cc_fork/scripts/fork.sh "<task-title>" "${CLAUDE_SESSION_ID}" "<prompt>"
   ```
