...
**Not for:** Beads that share files within the same wave (run those sequentially), work not tracked in beads.
...
2. Show each wave with its beads (from waves.sh output)
...
2. **Read close reasons** — `bd show <id>` for each closed bead; the close reason IS the result
3. **Downstream integrity check** — for each closed bead, compare close reason against original body:
...
4. **Commit** (if `--commit false`): `git add <files> && git commit -m "feat: <epic summary>"`
5. **Epic status**: `bd epic status`
6. **Close-eligible**: if all children done: `bd epic close-eligible`
7. **Sync + push**: `bd sync && git push`
