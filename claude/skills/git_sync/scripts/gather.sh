...
STATIC=$("$SCRIPT_DIR/gather-static.sh" "$TRUNK")
TRUNK=$(echo "$STATIC" | jq -r '.trunk')
GH_AVAILABLE=$(echo "$STATIC" | jq -r '.ghAvailable')

# Gather all components
WORKTREES=$("$SCRIPT_DIR/gather-worktrees.sh" "$TRUNK" "$GH_AVAILABLE")
BRANCHES=$("$SCRIPT_DIR/gather-branches.sh" "$TRUNK" "$GH_AVAILABLE")
TAGS=$("$SCRIPT_DIR/gather-tags.sh")
