---
name: git:worktree
description: Use when starting feature work that needs isolation from current workspace, when creating a git worktree, or when another skill needs an isolated workspace.
---
...
```bash
PATHS=$(bash scripts/resolve-paths "$BRANCH_NAME")
```
...
```bash
bash scripts/copy-env "$CURRENT_DIR" "$WORKTREE_DIR"
```

Auto-detects and copies common tool files. Run `scripts/copy-env --help` for the list. For project-specific files beyond what the script handles, copy them manually after the script runs.

### Step 4: Pre-Trust in Claude Code

```bash
bash scripts/pre-trust "$WORKTREE_DIR"
```
...
| Script          | Purpose                                                     |
| --------------- | ----------------------------------------------------------- |
| `resolve-paths` | JSON: `mainRepo`, `currentDir`, `worktreeDir`, `isWorktree` |
| `copy-env`      | Auto-detect + copy common gitignored files                  |
| `pre-trust`     | Pre-trust directory in `~/.claude.json`                     |

## Reference

- `reference/worktree-comparison.md` - When to use worktrees vs regular branches
