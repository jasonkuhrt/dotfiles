...
The `{workspace}` comes from core config resolution. The `{displayName}` is the user's `displayName` field, not their email or full name.
...
- Current context (via current-issue topic)
