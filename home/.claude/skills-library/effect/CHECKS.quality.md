...
Use `Option` combinators instead of null checks. `T | undefined` from `Schema.optional` should be wrapped at first access with `Option.fromNullable()`. The wrapping happens at the boundary where the nullable value enters domain logic — not deep inside a call chain.
...
## no-business-logic-on-schema

Schema classes own data operations: `is`, `make`, derived getters, orderings. Business logic (side effects, service calls, multi-entity orchestration) belongs in services, not on the class.
...
const sendConfirmation = (order: Order) => Effect.gen(function*() { ... })
```
...
export class Order extends S.TaggedClass<Order>('Order')('Order', {
  ...
  sendConfirmation() { ... }  // side effect — belongs in a service
  async save() { ... }        // persistence — belongs in a repository
}
```
