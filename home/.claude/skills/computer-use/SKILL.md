---
name: computer-use
description: Choosing the right GUI-automation layer on macOS and not fighting app tiering. Use before driving any native app, taking screenshots, or clicking/typing into a desktop UI — and whenever deciding between a dedicated MCP, bash/CLI, claude-in-chrome, peekaboo, macos-automator, or the bundled computer-use tools. Explains the tier order, why browsers are read-only and terminals click-only, the machine-wide lock, and TCC setup.
---
...
Opening or launching a closed app, or opening a file/URL in one, is benign and reversible. **Just do it** — never refuse, never ask permission.

| Action | Approval? |
|---|---|
| **Launch/open** a closed app (`open -a "Google Chrome"`), open a file or URL | **No — just do it** |
| **Quit / kill / restart** an app the user is running | **Yes — per occurrence** (destroys unsaved state) |
| Force-kill a process you don't own | **Never** |

Chrome closed and blocking browser work → `open -a "Google Chrome"` yourself. The quit-requires-approval rule is QUIT-scoped, never launch-scoped.

**The tell:** about to write "please open X and tell me when it's running" — stop, and run it yourself.
