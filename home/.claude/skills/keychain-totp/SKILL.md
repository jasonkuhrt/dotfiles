---
name: keychain-totp
description: Storing and reading credentials from the macOS keychain, and generating TOTP 2FA codes with oathtool. Use when automating a login, handling two-factor auth, or stashing a password, API key, or otpauth secret for later agent use. Covers why apw is not used, the Setup URL vs Recovery Key trap, and the two keychain entry types.
---
