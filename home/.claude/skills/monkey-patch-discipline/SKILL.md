---
name: monkey-patch-discipline
description: Safely extending or patching a third-party module at runtime. Use before assigning to, or adding a field on, any imported/required third-party object — in any language where attribute access can be intercepted (Lua metatables, JS Proxy, Python __getattr__, Ruby method_missing). Explains why you must read the module source first, why you may only REPLACE existing methods and never ADD new fields, and where owned state belongs instead.
---
