  ...
  questions: [
    {
      ...
      options: [
        {
          label: "here (Recommended)",
          description: "Files touched in this conversation, fallback to dirty",
        },
        {
          label: "dirty",
          description: "All uncommitted changes (staged + unstaged)",
        },
      ...
      options: [
        {
          ...
          description: "Gate + quality checks",
        },
    ...
    },
    {
      ...
      options: [
        {
          label: "All groups (Recommended)",
          description: "Run all relevant checks",
        },
...
4. **Resolve paths** — path precedence (first wins):
   1. `config.skills[skill].paths` from CLAUDE.md frontmatter
   2. `check.paths` from check file frontmatter (returned by discover.ts)
   3. Default: all project files
5. **Apply default exclusions** — always exclude: `generated/**`, `dist/**`, `node_modules/**`, `*.generated.*`
6. **Apply tier ceiling** — if TIER is a specific tier (`gate`, `quality`, `polish`), keep only checks at that tier. If TIER is `all`, keep gate + quality + polish in order.
7. **Apply scope** — intersect resolved paths (after exclusions) with the scoped file list from step 1

### 4a. LOG RESOLVED FILE LIST

**CRITICAL**: Before executing any checks, output the resolved file list per check to the conversation:

```
Resolved files per check:
- checks#no-non-null-assertions: 42 files (apps/**/*.ts, libs/**/*.ts)
- checks#function-expressions-preferred: 632 files (all project files)
```

This makes path filtering visible and debuggable. If a check has 0 files after intersection, skip it and report as N/A.
...
1. **Glob paths** — get files matching the check's resolved paths (already intersected with scope and exclusions)
...
**Subagent constraint**: If dispatching subagents for evaluation, each subagent prompt MUST include the explicit file list for its checks. Subagents must NOT scan freely — they evaluate only the files they are given.
