...
4. If a ready bead has the `flo/interactive` label, still show it as a selectable option but annotate it: **"⚡ Interactive — this bead requires human-in-the-loop and should not be delegated to a background agent."** Don't block selection — just inform.
5. If no ready beads: report to user — either all are blocked (show blockers), all are claimed (show who claimed them), or the epic is complete
