...
- **NEVER block the session with CI watch commands.** `gh pr checks --watch`, `gh run watch`, and similar polling commands MUST use `run_in_background: true`. A blocking watch freezes the entire conversation — the user cannot interact until it finishes or times out. There are NO exceptions to this rule.
