...
function _git_railway
    ...
    if _git_is_trunk
        ...
        set -l crop_half 5
        set -l commits (command git log -50 --format='%h|%s|%cr' 2>/dev/null)
        set -l total (count $commits)
        set -l cropping (test $total -gt $max_display && echo 1 || echo 0)
        ...
        for commit in $commits
            ...
            # Middle crop: skip hidden commits, show ellipsis once
            if test $cropping -eq 1
                if test $idx -eq (math $crop_half + 1)
                    set -l hidden (math $total - $max_display)
                    set_color brblack
                    printf "  │  ⋮ %d more\n" $hidden
                end
                if test $idx -gt $crop_half -a $idx -le (math $total - $crop_half)
                    continue
                end
            end
    ...
    set -l max_display 10
    set -l crop_half 5
    set -l cropping (test $branch_count -gt $max_display && echo 1 || echo 0)
    ...
    for commit in $branch_commit_list
        ...
        # Middle crop: skip hidden commits, show ellipsis once
        if test $cropping -eq 1
            if test $commit_idx -eq (math $crop_half + 1)
                set -l hidden (math $branch_count - $max_display)
                set_color brblack
                if test $diverged -eq 1
                    printf "  │    │  ⋮ %d more\n" $hidden
                else
            ...
            if test $commit_idx -gt $crop_half -a $commit_idx -le (math $branch_count - $crop_half)
                continue
            end
        end
